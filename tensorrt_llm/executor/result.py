...
class GenerationResultBase:
    ...
    def _tokens_trimmed_stop_sequence(self,
                                      output: CompletionOutput,
                                      use_tokens_diff: bool = False):
        ...
        if self.sampling_params.detokenize and self.tokenizer is not None:
            for beam_output in self.outputs:
                ...
                if hasattr(
                    ...
                    tokens_diff = self._tokens_trimmed_stop_sequence(
                        beam_output, use_tokens_diff=True)
                    ...
                    beam_output.text = self.tokenizer.decode(tokens, **kwargs)
...
def _process_req_perf_metrics(
    ...
    if spec_decode_acceptance_rate := req_perf_metrics_dict.get(
            "acceptance_rate"):
        stat[MetricNames.
             SPEC_DECODE_DRAFT_ACCEPTANCE_RATE] = spec_decode_acceptance_rate
    if spec_decode_accepted_tokens := req_perf_metrics_dict.get(
            "total_accepted_draft_tokens", 0):
        stat[MetricNames.
             SPEC_DECODE_ACCEPTED_TOKENS] = spec_decode_accepted_tokens
    if spec_decode_draft_tokens := req_perf_metrics_dict.get(
