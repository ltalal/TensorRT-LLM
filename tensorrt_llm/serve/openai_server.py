...
class OpenAIServer:
    ...
    def register_routes(self):
        ...
        self.app.add_api_route("/metrics/", self.metrics, methods=["GET"])
        self.app.add_api_route("/latest_iter_stats",
                               self.get_latest_iteration_stats,
                               methods=["GET"])
    ...
    async def read_metrics_file(self):
        ...
        if prom_metrics["request_started_total"] == all_requests_done:
            prom_metrics["num_requests_waiting"] = prom_metrics[
                "num_requests_running"] = 0
        else:
            # Detect number of requests not being processed by the TensorRT-LLM engine.
            prom_metrics["num_requests_waiting"] = max(
        ...
        if self.metrics_collector:
            ...
            self.metrics_collector.prompt_tokens_total.set(
                prom_metrics["prompt_tokens_total"])
            try:
                executor = getattr(self.generator, '_executor', None)
                stats_list = executor.get_stats(timeout=0) if executor else None
            except Exception as e:
                traceback.print_exc()
                logger.warning(f"openai_server.py: cannot get stats {e}")
                return

            if stats_list is not None:
                logger.debug(f"Received %d iter stats", len(stats_list))
            ...
            logger.debug(f"Iter stats", stats)
            ...
            self.metrics_collector.gpu_cache_blocks_missed_total.set(
                kv_stat["missedBlocks"])
            self.metrics_collector.gpu_cache_blocks_alloc_new_total.set(
                kv_stat["allocNewBlocks"])
            self.metrics_collector.gpu_cache_blocks_alloc_total.set(
                kv_stat["allocTotalBlocks"])
            self.metrics_collector.conf_kv_tokens_per_block.set(
                kv_stat["tokensPerBlock"])
    ...
    async def get_latest_iteration_stats(self) -> JSONResponse:
        return JSONResponse(content=self.latest_stats or [])
    ...
    async def openai_chat(self, request: ChatCompletionRequest,
        ...
        if (request.logprobs or request.top_logprobs
            ) and self.generator.args.speculative_config is not None:
            return self.create_error_response(
    ...
    async def openai_completion(self, request: CompletionRequest,
                                raw_request: Request) -> Response:
        # Validate logprobs is not requested with speculative decoding
        if request.logprobs and self.generator.args.speculative_config is not None:
            return self.create_error_response(
                message="logprobs is not supported with speculative decoding",
                err_type="BadRequestError",
                status_code=HTTPStatus.BAD_REQUEST)
